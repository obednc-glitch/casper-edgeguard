...
## Future direction: on-chain policy transparency

The current design anchors a hash-commitment of each compliance report, keeping verdicts private while still letting anyone verify authenticity by re-hashing and comparing on-chain. A natural next step is making the *rules* that produced the verdict auditable too - publishing versioned policy parameters on-chain (e.g. which OFAC list snapshot, jurisdiction risk thresholds, valuation tolerance bands) alongside the report hash. That would let a third party independently confirm a report was generated by a known, unmodified rule set, without ever exposing the report's contents. This wasn't in scope for the buildathon timeline, but fits naturally on top of the existing privacy-preserving architecture.
